<template>
    ...
    <v-container> <!-- 'v-container' est´substituindo ' div class="clubes-lista" '-->
        <div class="clubes-lista-separado-ordenado">
    
            <h2 class="text-h5 text-center mb-3 mt-5">Classificação dos clubes</h2>
            
            <ClubesTabela v-bind:clubesRecebidos="clubesListaOrdenada"/>

            <v-container>
                <p>
                    Foi passado a função clubesListaOrdenada para o componente filho, 
                    pois queremos a lista ordenada.

                </p>
                <p>
                    Foi tirado o v-container que envolve todos os elementos desse componente, 
                    pois quando chamamos o conponente o filho, ele já tem seu v-container, fazendo
                    com que a exibição da lista de clubes fique menor que nas outras telas 
                    que não usam componente filho.
